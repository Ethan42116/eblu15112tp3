...
#the question generated by the cuter
class Question():
...
#alignment: you start on the very left and adjust accordingly (done but bad code and can fine tune)
#correct: get confetti and maybe a sound? (brainstorm today)
#  maybe have a checkmark
#incorrect: get your written answer and the read answer (done)
#make report issue page (done)
#need to implement fraction (done)
#start page: maybe have like a mincraft like animation without load screen (maybe not)
    #include instructions in start page (done)
#need to make buttons prettyier
#need background color
#need coolors pallette
#use images for buttons



#gets an answer from raw AI output, which is an output
def getAnswer(app):
    answer=0
    factor=1
    ...
    answerString=""
    #.all() sources comes from compiler error when I used (app.predictionValue)==None
